...
var GitHubActivity = (function() {
  ...
  var methods = {
    ...
    getMessageFor: function(data) {
      ...
      data.userGravatar = Mustache.render('<span class="avatar avatar-sm mr-1" style="background-image: url({{url}})"></span>', { url: data.actor.avatar_url });
...
var templates = {
  Stream: '<div class="list list-row list-hoverable">{{{text}}}</div>',
  Activity: '<div class="list-item">\
               <div class="avatar avatar-md"><span class="octicon octicon-{{icon}}"></span></div>\
               <div class="text"><small class="d-block text-muted mt-1">{{{timeString}}}</small> <div class="text-body h4 d-inline-block mb-0">{{{userLink}}} {{{message}}}</div></div>\
               <div class="gha-clear"></div>\
             </div>',
  SingleLineActivity: '<div class="list-item">\
                         <div class="avatar avatar-md"><span class="octicon octicon-{{icon}}"></span></div>\
                         <div class="text"><small class="d-block text-muted mt-1">{{{timeString}}}</small><div class="text-body h4 d-inline-block mb-0">{{{userLink}}} {{{message}}}</div></div>\
                         <div class="gha-clear"></div>\
                       </div>',
  ...
  PushEvent: 'pushed to {{{branchLink}}}{{{repoLink}}}<br>\
                <ul class="list-unstyled mb-0">{{#payload.commits}}<li><small>{{{committerGravatar}}} {{{shaLink}}} {{message}}</small></li>{{/payload.commits}}</ul>\
